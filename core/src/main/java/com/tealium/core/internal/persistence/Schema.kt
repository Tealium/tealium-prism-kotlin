...
object Schema {
     ...
     * The "active" column denotes that the dispatcher is currently in use in the SDK.
     */
    object DispatcherTable {
        ...
        val CREATE_DISPATCHERS_TABLE = """
            CREATE TABLE IF NOT EXISTS $TABLE_NAME(
                 ...
                 $COLUMN_ACTIVE BOOLEAN DEFAULT true NOT NULL
            );
        """.trimIndent()
    }
     ...
     * the [QueueTable] for any Dispatcher set to active in the [DispatcherTable]
     */
        ...
        val CREATE_TRIGGER_ADD_TO_QUEUE = """
            ...
            BEGIN 
                INSERT INTO ${QueueTable.TABLE_NAME}
                    ...
                    WHERE ${DispatcherTable.TABLE_NAME}.${DispatcherTable.COLUMN_ACTIVE} = true;
            END;
        """.trimIndent()
    }

    /**
     * The "queue" table contains only the Dispatch uuid and the id's of active dispatchers in the
     * [DispatcherTable] at the time the Dispatch was added to the [DispatchTable].
     *
     * There will be an entry mapping each active Dispatcher to each Dispatch, to ensure that every
