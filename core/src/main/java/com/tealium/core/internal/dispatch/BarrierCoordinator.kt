...
interface BarrierCoordinator {
    ...
    /**
     * Returns a [Flow] of [BarrierState]s that are specific to the [Dispatcher] identified by the
     * [dispatcherId].
     *
     * The [BarrierState] values emitted from the returned flow are a combination of the states of
     ...
     * from the returned [Flow]. That is, the combined [BarrierState]s are all required to be
     ...
     * @return A [Flow] of [BarrierState]s for the [Dispatcher]
     */
    fun onBarriersState(dispatcherId: String): Flow<BarrierState>
}