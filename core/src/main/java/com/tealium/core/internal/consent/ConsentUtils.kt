...
fun Dispatch.applyDecision(decision: ConsentDecision): Dispatch? {
    ...
    this.addAll(DataObject.create {
        put(
            ...
            unprocessedPurposes.asDataList()
        )
        put(Dispatch.Keys.PURPOSES_WITH_CONSENT_PROCESSED, processedPurposes)
        put(Dispatch.Keys.PURPOSES_WITH_CONSENT_ALL, decision.purposes.asDataList())
        put(
...
fun Dispatch.matchesConfiguration(configuration: ConsentConfiguration, dispatcherId: String): Boolean {
    val consentedPurposes = payload().getDataList(Dispatch.Keys.PURPOSES_WITH_CONSENT_ALL)
