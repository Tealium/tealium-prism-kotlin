...
class TimeCollectorTests {
    ...
    private val timeCollector: TimeCollector by lazy {
        TimeCollector()
    }
    ...
    fun collect_Returns_Empty_Object_When_Timestamp_Epoch_Milliseconds_Missing_From_Context() {
        ...
        val collected = timeCollector.collect(dispatchContext)
    ...
    fun collect_Delegates_Date_Formats_To_Date_Formatter() {
        ...
        val collected = TimeCollector(formatter)
    ...
    fun collect_Returns_Correctly_Formatted_Iso8601_Local_Time() {
        ...
        val losAngelesTime = timeCollector.collect(dispatchContext())

        val katmandu = TimeZone.getTimeZone("Asia/Katmandu")
        TimeZone.setDefault(katmandu) // +05:45
        val katmanduTime = timeCollector.collect(dispatchContext())
    ...
    fun factory_Id_Matches_Module_Id() {
        val timeCollector = TimeCollector.Factory.create(mockk(), mockk())!!

        assertEquals(TimeCollector.Factory.id, timeCollector.id)
    }

    @Test
    fun factory_Creates_New_Instances() {
        val instance1 = TimeCollector.Factory.create(mockk(), mockk())
        val instance2 = TimeCollector.Factory.create(mockk(), mockk())
