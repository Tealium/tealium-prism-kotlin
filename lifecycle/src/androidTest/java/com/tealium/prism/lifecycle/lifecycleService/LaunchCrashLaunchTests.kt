...
class LaunchCrashLaunchTests : LifecycleServiceBaseTests() {
    ...
    private val customEventTimestamp
        get() = secondLaunchTimestamp + millisecondsPerDay + millisecondsPerHour
    ...
    fun didDetectCrash_isCorrect() {
        assertEquals(true, lifecycleEventState.getBoolean(StateKey.LIFECYCLE_DIDDETECT_CRASH))
    ...
    fun totalCrashCount_isCorrect() {
        assertEquals(1, lifecycleEventState.getInt(StateKey.LIFECYCLE_TOTALCRASHCOUNT))
        assertEquals(1, customEventState.getInt(StateKey.LIFECYCLE_TOTALCRASHCOUNT))
    }
